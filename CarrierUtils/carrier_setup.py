...
def __search_carriers(phone_number: str, carrier_dictionary: dict) -> str or list:
    ...
    :return: List close_matches of most similar to cell carrier from phone_number or phone_number's carrier
    """
    ...
    for key in keys:
        ...
        if user_carrier == key:
            # If they are the same string, return the string - we have found the correct cell carrier in our dict
            return user_carrier
        # Check if the current carrier string (key) and user_carrier contain each other
            ...
            return key
        # Check if either the current carrier string (key) or user_carrier contain or both contain a space
    ...
    for match in difflib.get_close_matches(user_carrier, list(carrier_dictionary.keys()), n=5, cutoff=0.2):
...
def setup(phone_number: str, carrier_dictionary: dict) -> str:
    ...
    :param carrier_dictionary: Dictionary object containing cell carriers and their text-to-email addresses
